 ...
 * */
const char *version          = "Application name: %s  Version 9.3 от 16.10.2018";
...
bool FileStatExists(char* str, std::string &s)
{
        ...
        if (stat(str, &sb) == -1)
        {
            m << COMMENT_INFO << errno << " " << (unsigned int)time(0) << " " << str << " : " << strerror(errno) << std::endl;
            s.append(m.str());
            return false;
        }
        ...
        if (!v)
        {
            m << COMMENT_INFO << ERR_FILE_EMPTY << " " <<  str << " : Something wrong (file is empty) with file" << endl;
            s.append(m.str());
        }
        return v;
}
