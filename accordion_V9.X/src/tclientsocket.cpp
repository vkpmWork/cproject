...
bool tclient_socket::net_connect()
{
    ...
    if(sock < 0)
    {
    	logger::message << errno << " " << time(0) << " Socket error : " << strerror(errno) << std::endl;
    	logger::write_log(logger::message);
...
*/
    ...
    if (!srv)
    {
    	logger::message << h_errno << " " << time(0) << " Socket error : " << hstrerror(errno) << std::endl;
    	logger::write_log(logger::message);
    	logger::write_info(logger::message);
    	return ret_value;
    }
    ...
    while (connection_counter < max_conections)
    {
    	...
    	else
    	{
    		...
    		connection_counter++;
                logger::message.str("");
                logger::message << "Connection attempt number " << connection_counter << " " << host << ":" << port << endl;
    ...
    if (ret_value != -1 && socket_type == SOCKET_NONBLOCK)
    {
			...
			if (socketFlags < 0)
			{
					logger::message << errno << " " << time(0) << " Impossible to retrieve socket descriptor flags : " << strerror(errno) << std::endl;
					logger::write_log(logger::message);
					logger::write_info(logger::message);
					return ret_value;
			}
			...
			{
					logger::message << errno << " " << time(0) << " Impossible to update socket descriptor flags : " << strerror(errno) << std::endl;
					logger::write_log(logger::message);
					logger::write_info(logger::message);
					return ret_value;
			}
        ...
        if(ret_value == -1)
        {
        	logger::message << errno << " " << time(0) << " Connection error : " << strerror(m_errno = errno) << " on host : " << host << " port : " << port << std::endl;
        	logger::write_log(logger::message);
			logger::write_info(logger::message);

        	//if (m_errno == 10037) ret_value = true;
        }
	...
	int total = 0, m_out = 0;
	  while(total < length)
	  {
	                  ...
	                  if(m_out == -1)
	                  {
	                          ...
	                          logger::message.str("");
	                          logger::message << errno << " " << time(0) <<  " : " << "net_send(const char *buf, int length) Transmit error : " << strerror(errno) << std::endl;
	                          logger::write_log(logger::message);
	                          logger::write_info(logger::message);
	                          f_connected = false;
	                          break;
