 ...
 *
 * 04.10.2018 - отправка фреймов net_send добавила строку if (errno == EAGAIN)...
*/
...
const char *str_process			 = "ps --sort=start_time -axo pid,command | grep %s | grep %s | grep %s | grep -v ps";
...
enum tcCmd {cEmpty = 0, cStop, cMake, cStatus};
/* =================== prototypes ================= */
void   ClearMemory();
...
int main(int argc, char **argv)
{
  ...
  if (argc <= 4)
    ...
    if (cmd != cEmpty)
    {
        	if (m_pid && (cmd != cStatus))
        	{
        		kill(m_pid, cmd == cStop ? SIGTERM : SIGUSR1);
    		...
    		if (cmd == cStatus || cmd == cMake) exit(m_pid != 0 );
    		else exit(m_pid == 0); /* процесс остановлен == 1*/
    }
