...
tconfig::tconfig(char *m_cfg)
        : pConfig(NULL)
        , log_on_start(NULL)
{
  m_cfg_file   = strdup(m_cfg);
  ReadOnStartIni();
}
...
void tconfig::ReadOnStartIni()
{
  ...
  std::string s = reader.Get(SECTION_MISC, "log_error_on_start", "");

  if (!s.empty()) log_on_start = strdup(s.c_str());
}
