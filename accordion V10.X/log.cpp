...
{
        if (ptr_log) ptr_log->write_marker(msg);
}
...
Tlog::Tlog(char* m_file, common::tcLogLevel m_level) :
	  log_file(NULL)
	, log_error(true)
	, log_level(m_level)
{
...
void Tlog::check_log_size(int fsize)
{
	...
	if (stat(log_file, &buf) == 0)
	{
		if (buf.st_size >= fsize)
		{
			...
			fwrite(header.str().c_str(), 1/*sizeof(char)*/, header.str().length(), outfile);
			if (e)
...
TStartLog::TStartLog(char* m_file): Tlog(m_file)
{
  is_error = m_file == NULL;
  if (!is_error) log_file  = strdup(m_file);
}
...
/* ========================= TInfoLog ================== */
TInfoLog::TInfoLog(char* m_file, char m_channel_type, int m_channel_id, int logsize, common::tcLogLevel m_level, char* m_info_marker) : Tlog(m_file, m_level)
	,	path(NULL)
	,	channel_type(m_channel_type)
	,	channel_id  (m_channel_id)
	, 	log_size(logsize)
        ,       info_marker(m_info_marker)
...
void TInfoLog::set_log_name()
{
	char *buffer = (char*)calloc(PATH_MAX, sizeof(char));
//	sprintf(buffer, "%sl%c%i.log", path, channel_type, channel_id); Убрала литеру 'l' по требованию Власова

	int len = strlen(path);
	if (path[len-1] == DELIMITER) sprintf(buffer, "%s%s/%c%i.log", path, common::GetSubPath(channel_id).c_str(), channel_type, channel_id);
	else sprintf(buffer, "%s/%s/%c%i.log", path, common::GetSubPath(channel_id).c_str(), channel_type, channel_id);
...
{
	std::string str;
	str.append(COMMENT_INFO);
	str.append(msg.str());

	write(str);
}

void     TInfoLog::write_marker(std::ostringstream &msg)
{
        std::string str;
        str.append(info_marker);
        str.append(" ");
        str.append(msg.str());
